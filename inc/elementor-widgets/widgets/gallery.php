...
/**
 *
 * Recipes elementor gallery section widget.
 *
 * @since 1.0
 */
class Recipes_Gallery extends Widget_Base {

	public function get_name() {
		return 'recipes-gallery';
	}

	public function get_title() {
		return __( 'Gallery', 'recipes-companion' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'gallery_content',
			[
				'label' => __( 'Gallery content', 'recipes-companion' ),
			]
        );
        $this->add_control(
            'sec_title',
            [
                ...
                'label_block' => true,
                'default' => esc_html__( 'Gallery.  Photos', 'recipes-companion' ),
            ]
        );
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__( 'Galleries', 'recipes-companion' ),
		...
		$this->add_control(
            'galleries', [
                ...
                'fields' => [
                    [
                        'name' => 'item_img',
                        'label' => __( 'Item Image', 'recipes-companion' ),
                        'label_block' => true,
                    ...
                    [
                        'name' => 'img_size',
                        'label' => __( 'Select Image Size', 'recipes-companion' ),
                        'label_block' => true,
                        'type' => Controls_Manager::SELECT,
                        'default' => 'recipes_gallery_thumb_482x410',
                        'options' => [
                            'recipes_gallery_thumb_601x410' => '601x410',
                            'recipes_gallery_thumb_482x410' => '482x410',
                            'recipes_gallery_thumb_721x410' => '721x410',
                        ]
                    ],
                ],
                'default'   => [
                    [      
                        'img_size'    => 'recipes_gallery_thumb_601x410',
                        'item_img'    => [
                            'url'        => Utils::get_placeholder_image_src(),
                        ],
                    ],
                    [      
                        'img_size'    => 'recipes_gallery_thumb_482x410',
                        'item_img'    => [
                            'url'        => Utils::get_placeholder_image_src(),
                        ],
                    ],
                    [      
                        'img_size'    => 'recipes_gallery_thumb_721x410',
                        'item_img'    => [
                            'url'        => Utils::get_placeholder_image_src(),
                        ],
                    ],
            ...
            <div class="row grid no-gutters">
                ...
                if( is_array( $galleries ) && count( $galleries ) > 0 ) {
                    foreach( $galleries as $item ) {
                        ...
                        // Get the img size
                        switch ($img_size) {
                            case 'recipes_gallery_thumb_482x410':
                                $dynamic_class = 'col-xl-3 ';
                                break;
                            case 'recipes_gallery_thumb_721x410':
                        ...
                        <div class="<?=esc_attr($dynamic_class)?>col-md-6 col-lg-6 grid-item">
                            <div class="single_gallery gallery_imag_<?=esc_attr($i)?>" <?php echo recipes_inline_bg_img( esc_url( $img_url ) ); ?>>
