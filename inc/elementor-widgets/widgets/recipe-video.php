...
/**
 *
 * Recipes elementor Recipes video section widget.
 *
 * @since 1.0
 */
class Recipes_Recipe_Video extends Widget_Base {

	public function get_name() {
		return 'recipes-recipe-video';
	}

	public function get_title() {
		return __( 'Recipe Video', 'recipes-companion' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Recipe Video content', 'recipes-companion' ),
			]
        );
        $this->add_control(
            'sec_title',
            [
                ...
                'label_block' => true,
                'default'   => esc_html__( 'Recipe videos that never misses any portion', 'recipes-companion' ),
            ]
        );
        $this->add_control(
            'sec_text',
            [
                ...
                'type' => Controls_Manager::TEXTAREA,
                'label_block' => true,
                'default'   => esc_html__( 'inappropriate behavior is often laughed off as “boys will be boys,” women face higher conduct standards especially in the workplace. That’s why it’s crucial that, as women.', 'recipes-companion' ),
            ]
        );
        $this->add_control(
            'video_label_1',
            [
                'label' => esc_html__( 'Video Label 1', 'recipes-companion' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default'   => esc_html__( 'Watch Video', 'recipes-companion' ),
            ]
        );
        $this->add_control(
            'video_label_2',
            [
                'label' => esc_html__( 'Video Label 2', 'recipes-companion' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default'   => esc_html__( 'You will love our execution', 'recipes-companion' ),
            ]
        );
        $this->add_control(
            'video_url',
            [
                'label' => esc_html__( 'Video URL', 'recipes-companion' ),
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__( 'Right Sections', 'recipes-companion' ),
        ...
        $this->add_control(
            'sec_img',
            [
                'label' => esc_html__( 'Big Image', 'recipes-companion' ),
                'description' => esc_html__( 'Best size is 457x480', 'recipes-companion' ),
        ...
        $this->add_control(
            'left_img',
            [
                'label' => esc_html__( 'Left Small Image', 'recipes-companion' ),
                'description' => esc_html__( 'Best size is 205x230', 'recipes-companion' ),
                'type' => Controls_Manager::MEDIA,
                'label_block' => true,
                'default'     => [
                    'url'   => Utils::get_placeholder_image_src(),
        ...
        $this->add_control(
            'bottom_img',
            [
                'label' => esc_html__( 'Bottom Small Image', 'recipes-companion' ),
                'description' => esc_html__( 'Best size is 205x230', 'recipes-companion' ),
                'type' => Controls_Manager::MEDIA,
                'label_block' => true,
                'default'     => [
    ...
    $sec_img = !empty( $settings['sec_img']['id'] ) ? wp_get_attachment_image( $settings['sec_img']['id'], 'recipes_video_big_thumb_457x480', '', array( 'alt' => $sec_title ) ) : '';
    $left_img = !empty( $settings['left_img']['id'] ) ? wp_get_attachment_image( $settings['left_img']['id'], 'recipes_video_small_thumb_205x230', '', array( 'alt' => $sec_title ) ) : '';
    $bottom_img = !empty( $settings['bottom_img']['id'] ) ? wp_get_attachment_image( $settings['bottom_img']['id'], 'recipes_video_small_thumb_205x230', '', array( 'alt' => $sec_title ) ) : '';
    ?>
